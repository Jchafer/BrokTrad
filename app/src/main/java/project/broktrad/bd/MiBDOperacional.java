...
public class MiBDOperacional implements Serializable {


    private MiBD miBD;

    protected MiBDOperacional(Context context){
        miBD = MiBD.getInstance(context);
    }

    private static MiBDOperacional instance = null;

    //***************************************
    // Interfaz publica de la API de las gasolineras
    //***************************************

    // Constructor de las gasolineras. Obtiene una instancia del mismo para operar
    public static MiBDOperacional getInstance(Context context) {
        if(instance == null) {
            instance = new MiBDOperacional(context);
        }
        return instance;
    }
    ...
    public Usuario login(Usuario c){
        ...
        if(aux==null){
            return null;
        }else if (aux.getClave().equals(c.getClave())){
            return aux;
        }else{
            return null;
        }
